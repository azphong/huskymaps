...
public class GenerativeSeamFinder implements SeamFinder {
    ...
    private static class PixelGraph implements Graph<Node> {
        ...
        public class Pixel implements Node {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                List<Edge<Node>> result = new ArrayList<>();
                //If pixel is on right edge, return one edge to sink.
                if(x == picture.width() - 1){
                    return List.of(new Edge<>(this, sink, 0));
                }
                //Otherwise, add edge to middle right neighbor.
                Pixel middle = new Pixel(x+1, y);
                result.add(new Edge<>(this, middle, f.apply(picture, x+1, y)));
                //If not on the top edge of picture, add edge to top right neighbor.
                if(y > 0){
                    Pixel top = new Pixel(x+1, y-1);
                    result.add(new Edge<>(this, top, f.apply(picture, x+1, y-1)));
                }
                //If not on bottom edge of picture, add edge to bottom right neighbor.
                if(y < picture.height() - 1){
                    Pixel bottom = new Pixel(x+1, y+1);
                    result.add(new Edge<>(this, bottom, f.apply(picture, x+1, y+1)));
                }
                return result;
            }
